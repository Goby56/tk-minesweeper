    ...
    def solve_board(self):
        ...
        while not self.board_is_solved():
            if start_pair_scan:
                ...
                start_pair_scan = False
                continue
            if self.search_simple_openings() == False: # No more easy openenings
                start_pair_scan = True
        print(self)

    def search_simple_openings(self):
    ...
    def pair_search(self):
        for row1, column1 in Utils.get_matrix_indicies(self.rows, self.columns):
            ...
            for row2, column2 in Utils.get_surrounding_indices(row1, column1, self.rows, self.columns):
                ...
                if value_a - value_b == len(unopened_a - unopened_b):
